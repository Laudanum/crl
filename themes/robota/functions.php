...
function xref_shortcode_publications( $attrs = array() ){
    $post_id = get_the_ID();
    $post_ids = explode(",", is_xref_get_list( $post_id ));

    // optional categories
    $category = $attrs['category'] ? $attrs['category'] : null;
    if ( $category ) $category = get_cat_ID( $category);

    // lets get the posts all in one hit for efficiency and filtering
    $args = array(
        'posts_per_page'    => -1,
        'category'          => $category,
        'post__in'          => $post_ids,
    ...
    if( $list == 'list' ){
        if( is_array($posts)){
            ...
            foreach( $posts as $post ){
                ...
                $str .= '<a href="'. get_permalink( $post ) . '">' . $post->post_title . '</a>';
                $i++;
            }
            return $str;
        }
